  ...
  partition(input, 4).forEach(async (indexes, partitionIndex) => {
    for (const index of indexes) {
      ...
      try {
        ...
        res = await res.text()
        if (res.includes('We do not have example sentences for')) {
          output_[element] = null
        } else {
          const parser = new DOMParser();
          const doc = parser.parseFromString(res, "text/html");
          res = doc.querySelector('table').querySelectorAll('tr[data-testid="example-row"]')
          res = Array.from(res)
          res = res.map(x => ({ es: x.querySelector('div[lang="es"]').textContent, en: x.querySelector('div[lang="en"]').textContent }))
