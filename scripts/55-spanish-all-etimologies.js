...
// etimologias_cache = etimologias_cache.map(([v, keys]) => ({ keys, v }))
...
etimologias_with_cache_path = '/home/srghma/projects/anki-cards-from-pdf/etimologias_cache.json'
etimologias_cache_orig = JSON.parse(fs.readFileSync(etimologias_with_cache_path).toString()); null
R.keys(etimologias_cache_orig).length

// etimologias_with_ru_cache.filter(x => x.value.match(/zorap/gi))
...
etimologias_cache__urls = JSON.parse(fs.readFileSync('/home/srghma/projects/anki-cards-from-pdf/html/spanish/etimologias_cache--urls--without-nulls.json'))
// etimologias_with_ru_cache = etimologias_with_ru_cache.filter(x => x.ruText.includes('La palabra'))
// fs.writeFileSync('/home/srghma/projects/anki-cards-from-pdf/etimologias_with_ru_cache.json_', JSON.stringify(etimologias_with_ru_cache, undefined, 2))

untranslated = R.omit(R.uniq(etimologias_with_ru_cache.map(x => [x.id, x.keysUniqFromValues, x.allKeys]).flat().flat()), R.filter(x => x, etimologias_cache_orig)); null
R.keys(untranslated).length
// delete etimologias_cache_orig['tundir-']

etimologias_cache = R.map(R.prop('etimology'), untranslated); null
etimologias_cache = R.toPairs(etimologias_cache)
etimologias_cache = etimologias_cache.filter(([k, v]) => v)
etimologias_cache = etimologias_cache.map(([k, v]) => [k, v.replace(/<i>- Gracias: [^<]*<\/i>/g, '')])
etimologias_cache = etimologias_cache.map(([k, v]) => [k, v.replace(/<!--[^>]+>/g, '')])
etimologias_cache = etimologias_cache.map(([k, v]) => [k, v.replace(/<h3>\n\s*/g, '<h3>')])
etimologias_cache = etimologias_cache.map(([k, v]) => [k, v.replace(/\s*\n\s*<\/h3>/g, '</h3>')])
etimologias_cache = etimologias_cache.map(([k, v]) => [k, v.replace(/<p>\n\s*/g, '<p>')])
etimologias_cache = etimologias_cache.map(([k, v]) => [k, v.replace(/\s*\n\s*<\/p>/g, '</p>')])
etimologias_cache = etimologias_cache.map(([k, v]) => {
  dom.window.document.body.innerHTML = v
  const remove = x => x.parentNode.removeChild(x)
  const removeAll = xs => Array.from(xs).forEach(remove)
  removeAll(dom.window.document.body.querySelectorAll('script'))
  removeAll(dom.window.document.body.querySelectorAll('ins'))
  return [k, dom.window.document.body.innerHTML]
})
etimologias_cache = etimologias_cache.map(([k, v]) => [k, v.replace(/\n+/g, '\n')])
etimologias_cache = etimologias_cache.map(([k, v]) => [k, v.replace(/<p align="center">\n<\/p>/g, '')])
etimologias_cache_1 = etimologias_cache

beautify_html = require('js-beautify').html
// prettify = require('html-prettify')
etimologias_cache = etimologias_cache_1.map(([k, v]) => [k, beautify_html(v)])
etimologias_cache = etimologias_cache.filter(([k, v]) => k !== 'http')
etimologias_cache = etimologias_cache.map(([k, v]) => ({ k, v }))
etimologias_cache = R.groupBy(R.prop('v'), etimologias_cache);null
etimologias_cache = R.values(etimologias_cache)
etimologias_cache = etimologias_cache.map(xs => ({ value: xs[0].v, keysUniqFromValues: R.uniq(xs.map(x => x.k).sort()) }))
...
etimologias_cache = etimologias_cache.map(x => {
  // try {
    const keysFromText = x.keysFromText.split(' ')
    let allKeys = [...x.keysUniqFromValues, ...keysFromText].flat()
    const removeAccent = str => str.normalize("NFD").replace(/\p{Diacritic}/gu, "")
    ...
    const originalUrls = R.uniq(allKeys.map(x => etimologias_cache__urls[x.toLowerCase()]).filter(R.identity))
    return { ...x, allKeys, originalUrls }
  // } catch (e) {
  //   console.log(x)
  // }
})
...
output__.forEach((x, index) => {
  console.log(`rm -f /home/srghma/Downloads/etymology-${index}.csv && ssconvert /home/srghma/Downloads/etymology-${index}\\ \\(1\\).xlsx /home/srghma/Downloads/etymology-${index}.csv`)
})
...
etimologias_cache_with_ru__old = JSON.parse(fs.readFileSync('/home/srghma/projects/anki-cards-from-pdf/html/spanish/etimologias_with_ru_cache.json'))
// etimologias_cache_with_ru__old.filter(x => x.id.length > 1)
